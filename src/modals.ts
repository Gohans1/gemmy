...
export class SetFocusMusicModal extends Modal {
	dataManager: DataManager;
	...
	onOpen() {
		const { contentEl } = this;
		contentEl.empty();
		contentEl.createEl("h2", { text: UI_TEXT.TITLES.SET_FOCUS_MUSIC });

		let url = this.dataManager.settings.focusMusicUrl || "";

		new Setting(contentEl)
			.setName(UI_TEXT.LABELS.FOCUS_MUSIC_URL_NAME)
			.setDesc(UI_TEXT.LABELS.FOCUS_MUSIC_URL_DESC)
			.addText((text) => {
		...
		new ButtonComponent(buttonContainer)
			...
			.setCta()
			.onClick(async () => {
				await this.dataManager.updateSettings({ focusMusicUrl: url });
				new Notice("Focus music updated!");
				this.close();
			});
	}
